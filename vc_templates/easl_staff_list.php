...
wp_enqueue_style('easl-staff-list-style',
    get_stylesheet_directory_uri() . '/assets/css/community.css');

wp_enqueue_script('easl-staff-list-script',
...
if ( $wpex_query->have_posts() ) :
    ...
    $output .= '<div class="easl-staff-list-block '. esc_attr( $wrap_classes ) .'"'. vcex_get_unique_id( $unique_id ) .'>';
    ...
    if ( 'true' == $filter && ! empty( $filter_terms ) ) {
        ...
        $filter_html .= '<div class="filter-wrapper"><ul class="'. $filter_classes .'"'. $filter_style .'>';

        if ( 'true' == $filter_all_link ) {

            $filter_html .= '<li';
            if ( ! $filter_has_active_cat ) {
                $filter_html .= ' class="active"';
            }
            $filter_html .= '>';

            $filter_html .= '<a href="#" data-filter="*" class="'. $filter_button_classes .'"><span>'. esc_html( $all_text ) .'</span></a>';
        ...
        foreach ( $filter_terms as $term ) :
            $staff_category_order = get_field('staff_category_order', $term);
            $term_array[$staff_category_order] = $term;
        endforeach;
        ...
        foreach ( $term_array as $term ) :
            ...
            $filter_html .= '<li class="filter-cat-'. $term->term_id;
            ...
            $filter_html .= '<a href="#" data-filter=".cat-'. $term->term_id .'" class="'. $filter_button_classes .'">';
            $filter_html .= $term->name;
            $filter_html .= '</a></li>';
            ...
            $count=0;
            $post_html .= '<div class="wpex-row easl-staff-list term-name cat-'. $term->term_id .'">'.
                            '<h2>'.$term->name.'</h2>';
            $post_html .= $term->description ? '<p>'.$term->description.'</p>' : '';
            $post_html .= '</div>';

            $post_html .= '<div class="'. $grid_classes .' easl-staff-list cat-'. $term->term_id .'"'. $grid_data .'>';
...
//                        }
        ...
        if ( $vcex_after_grid_filter = apply_filters( 'vcex_after_grid_filter', '', $atts ) ) {
            $filter_html .= $vcex_after_grid_filter; // @todo remove vcex_after_grid_filter filter and instead use a filer to override the whole output of the filter.
        }
